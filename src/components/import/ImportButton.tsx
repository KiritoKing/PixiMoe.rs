...
import { useImportFiles, useBatchAITagging } from "@/lib/hooks";
...
export function ImportButton() {
  ...
  const [thumbnailProcessingHashes, setThumbnailProcessingHashes] = useState<Set<string>>(new Set());
  ...
  const [aiTaggingProcessingHashes, setAiTaggingProcessingHashes] = useState<Set<string>>(new Set());
      ...
      if (payload.file_hash) {
        setAiTaggingProcessingHashes((prev) => {
          ...
          if (payload.stage === "classifying" || payload.stage === "saving_tags") {
            next.add(payload.file_hash!);
          } else if (payload.stage === "complete" || payload.stage === "error" || payload.stage === "skipped") {
            next.delete(payload.file_hash!);
            if (payload.stage === "complete") {
  ...
  const handleImportConfirm = async (options: { tagNames: string[]; enableAITagging: boolean }) => {
    ...
    try {
      ...
      if (options.enableAITagging && importedHashes.length > 0) {
        console.log("[Import] Starting batch AI tagging for", importedHashes.length, "files");
        ...
        <Popover open={hasContent}>
          ...
          <PopoverContent className="w-[320px] p-4" align="start" sideOffset={8}>
            <div className="space-y-3">
              ...
              {/* Thumbnail progress */}
              {(thumbnailProcessingHashes.size > 0 || (thumbnailCompleteCount > 0 && thumbnailCompleteCount < thumbnailTotal)) && (
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Loader2 className="h-3 w-3 animate-spin text-purple-600" />
                    <p className="text-xs font-medium text-purple-600">缩略图</p>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {thumbnailCompleteCount}/{thumbnailTotal} 完成
                    {thumbnailProcessingHashes.size > 0 && ` (${thumbnailProcessingHashes.size} 处理中)`}
                  </p>
                </div>
              )}
              
              {/* AI tagging progress */}
              {(aiTaggingProcessingHashes.size > 0 || (aiTaggingCompleteCount > 0 && aiTaggingCompleteCount < aiTaggingTotal)) && (
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Loader2 className="h-3 w-3 animate-spin text-green-600" />
                    <p className="text-xs font-medium text-green-600">AI 标签</p>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {aiTaggingCompleteCount}/{aiTaggingTotal} 完成
                    {aiTaggingProcessingHashes.size > 0 && ` (${aiTaggingProcessingHashes.size} 处理中)`}
                  </p>
                </div>
              )}
            </div>
