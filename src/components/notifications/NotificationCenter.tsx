...
import { Bell, X, CheckCircle2, AlertCircle, Info, AlertTriangle, Trash2, ChevronDown, ChevronUp } from "lucide-react";
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <Icon className={cn("mt-0.5 h-5 w-5 flex-shrink-0", typeColors[notification.type])} />
...
export function NotificationCenter() {
  ...
  const filterButtons: Array<{ label: string; value: NotificationType | "all" }> = [
    { label: "全部", value: "all" },
            ...
            {filteredNotifications.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  {notifications.length === 0 ? "暂无通知" : "没有符合条件的通知"}
                </p>
              </div>
            ) : (
              <div>
